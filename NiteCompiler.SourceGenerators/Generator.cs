...
public class Generator : IIncrementalGenerator
{
		...
		writer.EnterScope("{");
		{
			...
			writer.EnterScope("{");
			{
				...
				foreach (SyntaxKind kind in content.SyntaxKinds)
				{
					if (kind.Text != null)
					{
						writer.WriteLine($"SyntaxKind.{kind.Name} => \"{EscapeCsString(kind.Text)}\",");
					}
				}
	...
	private static string WriteNodeFile(NodeKind kind, HashSet<string> nodeTypeNames)
	{
		...
		writer.EnterScope("{");
		{
			...
			{
				string baseArgs = string.Join(", ", baseMembers.Select(m => ToCamelCase(m.Name)));
			...
			{
				writer.WriteLine();
				string visitName = GetVisitName(kind);
	...
	private static string EscapeCsString(string text)
	{
		return text.Replace("\\", "\\\\").Replace("\"", "\\\"").Replace("\n", "\\n").Replace("\r", "\\r").Replace("\t", "\\t");
	}
}